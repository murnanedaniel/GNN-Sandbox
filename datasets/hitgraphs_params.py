...
def load_graph(filename):
    with np.load(filename) as f:
        x, y_edges, y_params, pid = f['X'], f['y_edges'], f['y_params'], f['pid']
    ...
    return x, edge_index, y_edges, y_params, pid
...
class HitGraphDataset(Dataset):
    ...
    def __getitem__(self, index):
        x, edge_index, y_edges, y_params, pid = load_graph(self.filenames[index])
        # Compute weights
        w = y * self.real_weight + (1-y) * self.fake_weight
        return torch_geometric.data.Data(x=torch.from_numpy(x),
                                         edge_index=torch.from_numpy(edge_index), y_edges=torch.from_numpy(y_edges), 
                                         y_params=torch.from_numpy(y_params), w=torch.from_numpy(w), pid=torch.from_numpy(pid))
